...
class MirrorEngine:
    ...
    def initialize(self):
        ...
        try:
            ...
            self.logger.critical(
                f"Error while logging into Github, double-check credentials or try again later. {e}")
            sys.exit()
        ...
        if not os.path.isdir(config.local_repo_directory):
            ...
            try:
                ...
                self.logger.critical(f"An error occurred during cloning. {e}")
                sys.exit()
    ...
    def update(self):
        try:
            ...
            self.logger.critical(
                f"Error while obtaining upstream repository info, check if owner and repo names were entered correctly. {e}")
            return False
        ...
        for repo, event in stream_events.github_event_stream(self.github_api, [self.upstream, self.downstream],
            ...
            if event.type == "PullRequestEvent" and repo == self.upstream:
                ...
                if action.startswith("remirror"):
                    if event.payload['comment']['author_association'] != "MEMBER" \
                            and event.payload['comment']['author_association'] != "OWNER":
